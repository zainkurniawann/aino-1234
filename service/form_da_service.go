...
func generateFormNumberDA(documentID int64, divisionCode string, recursionCount int) (string, error) {
	...
	if count > 0 {
		...
		return generateFormNumberDA(documentID, divisionCode, recursionCount+1)
	}
...
func AddDA(addDA models.Form, isPublished bool, username string, userID int, divisionCode string, recrusionCount int, data models.DampakAnalisa, signatories []models.Signatory) error {
	...
	formNumberDA, err := generateFormNumberDA(documentID, divisionCode, recrusionCount+1)
	if err != nil {
		// Handle error
...
func FormDAByDivision(divisionCode string) ([]models.Formss, error) {
	var forms []models.Formss
	...
	for rows.Next() {
		var form models.Formss
		var projectName, reason, namaAnalis, jabatan, departemen, jenisPerubahan sql.NullString
		var detailDampak, rencanaPengembangan, rencanaPengujian, rencanaRilis sql.NullString

		err := rows.Scan(
			&form.FormUUID, &form.FormNumber, &form.FormTicket, &form.FormStatus,
			&form.DocumentName,
			&projectName, // NULL handling
			&reason, // NULL handling
			&form.CreatedBy, &form.CreatedAt, &form.UpdatedBy, &form.UpdatedAt, &form.DeletedBy, &form.DeletedAt,
			&namaAnalis, &jabatan, &departemen, &jenisPerubahan, &detailDampak,
			&rencanaPengembangan, &rencanaPengujian, &rencanaRilis,
			&form.ApprovalStatus,
		)
		if err != nil {
		...
		// form.Reason = getStringOrDefault(reason)
		form.NamaAnalis = getStringOrDefault(namaAnalis)
		form.Jabatan = getStringOrDefault(jabatan)
		form.Departemen = getStringOrDefault(departemen)
		form.JenisPerubahan = getStringOrDefault(jenisPerubahan)
		form.DetailDampakPerubahan = getStringOrDefault(detailDampak)
		form.RencanaPengembanganPerubahan = getStringOrDefault(rencanaPengembangan)
		form.RencanaPengujianPerubahanSistem = getStringOrDefault(rencanaPengujian)
		form.RencanaRilisPerubahanDanImplementasi = getStringOrDefault(rencanaRilis)

		forms = append(forms, form)
	}

	if len(forms) == 0 {
		log.Print("No records found for divisionCode:", divisionCode)
...
func GetDACode() (models.DocCodeName, error) {
