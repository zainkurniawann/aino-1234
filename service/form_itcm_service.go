...
func FormITCMByDivision(divisionCode string) ([]models.FormsITCM, error) {
	...
	rows, err := db.Query(`
		...
		WHERE d.document_code = 'ITCM' AND f.deleted_at IS NULL AND SPLIT_PART(f.form_number, '/', 2) = $1
		ORDER BY f.form_number DESC;
	`, divisionCode)
	...
	for rows.Next() {
		var form models.FormsITCM
		var projectName, projectManager sql.NullString // Gunakan sql.NullString untuk menangani NULL

		err := rows.Scan(
			&form.FormUUID, &form.FormNumber, &form.FormTicket, &form.FormStatus,
			&form.DocumentName,
			&projectName, &projectManager, // Gunakan sql.NullString
			&form.ApprovalStatus,
			&form.Reason, &form.CreatedBy, &form.CreatedAt, &form.UpdatedBy, &form.UpdatedAt,
			&form.DeletedBy, &form.DeletedAt,
			&form.NoDA, &form.NamaPemohon, &form.Intansi,
			&form.Tanggal, &form.PerubahanAset, &form.Deskripsi,
		)
		if err != nil {
		...
		form.ProjectManager = getStringOrDefault(projectManager)
